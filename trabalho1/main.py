...
def consulta(inventario):
    ...
    if not ativo_encontrado:
        for id_ativo, objeto_ativo in inventario.items():
            if objeto_ativo.nome.lower() == termo.lower():
                ativo_encontrado = objeto_ativo
    ...
    if ativo_encontrado:
        ...
        print(f"Tipo: {ativo_encontrado.__class__.__name__}")
...
def consultar_vul(inventario):
    ...
    if id_busca in inventario:
        ...
        if len(lista_vul) == 0:
            ...
            for id_vuln, vul in enumerate(lista_vul, start=1):
                print(f"{id_vuln}. Descrição: {vul['descricao']} | Gravidade: {vul['gravidade']}")
    else:
